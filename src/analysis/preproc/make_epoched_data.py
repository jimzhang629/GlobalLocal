...
def bandpass_and_epoch_and_find_task_significant_electrodes(sub, task='GlobalLocal', times=(-1, 1.5),
    ...
    - stat_func (function, optional): The statistical function to use for significance testing. Default is ieeg.calc.fast.ttest, time_perm_cluster's own default. It is Welch's t, identical to scipy's ttest_ind(equal_var=False) statistic (verified to ~6e-15), but vectorized over NaNs. scipy's nan_policy='omit' abandons its vectorized path the moment a single NaN appears and loops over 1-D slices in Python instead, which costs ~35-55x once outliers_to_nan or max_abs_z puts NaNs in the data.
