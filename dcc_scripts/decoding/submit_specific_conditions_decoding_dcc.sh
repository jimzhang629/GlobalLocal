...
#      the power-trace figures are drawn from (run_power_traces_dcc.py with
#      ANOVA_UNIT=electrode). Convert a run into the same labels table -- a pure
#      read of its summary.csv, seconds, no SLURM:
#
...
#      ANOVA_LABEL_EFFECT=congruency_only decodes exactly the electrodes
#      PLOT_SETS=congruency_only draws on the brain. Use ANOVA_LABEL_CORRECTION=flags
#      to keep the run's own thresholding.
#
#      Circularity: selecting on the congruency main effect and then decoding
#      congruency is the diagonal cell (docs/nested_electrode_selection.md); the
#      off-diagonal cells need no correction. ELECTRODE_DEFINITION_SPLIT in
#      run_decoding_dcc.py is the nested-selection fix for the diagonal ones.
